...
def dock_cmd(args):
    ...
    """
    import tkinter as tk
    import os, subprocess, pythoncom, win32com.client, win32gui, win32con, win32ui, time, threading
    ...
    if args and args[0].lower() == "kill":
        if dock_window and dock_window.winfo_exists():
            ...
            print("⚠️ No dock is currently open.")
        return
    ...
    def run_dock():
        ...
        def safe_image(path, size):
            """Try to load image from path or fallback."""
            try:
        ...
        globals()["dock_window"] = root  # link for kill command
        ...
        def build():
            ...
            for folder in sorted(os.listdir(DOCKITEMS_DIR)):
                ...
                # Find launch file and icon file
                launch_file = None
                icon_file = None
                ...
                if not launch_file:
                    continue  # skip if no executable/shortcut
