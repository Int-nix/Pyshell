...
icons_frame.pack(expand=True, fill="both", padx=5, pady=5)
...
tooltip_label = tk.Label(tooltip, text="", bg="#333333", fg="white", font=("Arial", 10))
...
def show_tooltip(btn, text):
    ...
    x = root.winfo_x() - tooltip.winfo_reqwidth() - 5
    y = btn.winfo_rooty() + (btn.winfo_height() - tooltip.winfo_reqheight()) // 2
...
for app in STATIC_APPS:
    ...
    btn = tk.Button(
        icons_frame, image=normal, bg=BG_COLOR, bd=0, highlightthickness=0,
        activebackground=BG_COLOR, command=lambda c=app["cmd"]: launch(c)
    )
    btn.pack(pady=SPACING // 2)
...
def resize_dock():
    ...
    dock_width = ICON_SIZE + 20
    dock_height = icons_frame.winfo_reqheight() + 20
    x = root.winfo_screenwidth() - dock_width - 10
    y = (root.winfo_screenheight() - dock_height) // 2
...
def slide_hide():
    x = root.winfo_x()
    target_x = root.winfo_screenwidth()
    if x < target_x:
        x = min(x + ANIMATION_SPEED, target_x)
...
def slide_show():
    root.deiconify()
    resize_dock()
    target_x = root.winfo_screenwidth() - root.winfo_width() - 10
    y = root.winfo_y()
    x = root.winfo_x()
    if x > target_x:
        x = max(x - ANIMATION_SPEED, target_x)
        root.geometry(f"+{x}+{y}")
...
def schedule_hide():
    ...
    hide_after_id = root.after(7000, hide_dock)
...
def check_mouse():
    try:
        ...
        if x >= screen_w - HOVER_AREA_WIDTH:
            show_dock()
            cancel_hide()
        elif dock_x1 <= x <= dock_x2 and dock_y1 <= y <= dock_y2:
            cancel_hide()
        else:
...
def update_windows():
    ...
    for hwnd, title in windows:
        ...
        btn = tk.Button(
            icons_frame, image=icon, bg=BG_COLOR, bd=0, highlightthickness=0,
            activebackground=BG_COLOR, command=lambda h=hwnd: activate_window(h)
        )
        btn.image = icon
        btn.pack(pady=SPACING // 2)
